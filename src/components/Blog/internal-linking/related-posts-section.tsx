...
export default function RelatedPostsSection({
  ...
  const gridClassName = isFeaturedSingle
    ? showHeader
      ...
      : ""
    : filtered.length === 2
      ? `${showHeader ? "mt-6 " : ""}grid gap-5 lg:grid-cols-2`
      : `${showHeader ? "mt-6 " : ""}grid gap-5 lg:grid-cols-3`;
  ...
  return (
    <section
      className={
        isEmbedded
          ? ""
          : "rounded-[1.75rem] border border-slate-200/80 bg-[linear-gradient(180deg,_rgba(248,250,252,0.9)_0%,_#ffffff_100%)] p-5 shadow-[0_18px_48px_rgba(15,23,42,0.06)] sm:p-6"
      }
      aria-labelledby={showHeader ? "related-posts-heading" : undefined}
      aria-label={showHeader ? undefined : "Articole înrudite"}
    >
      {showHeader ? (
        ...
        {filtered.map((p) => {
          ...
          return (
            <li key={p._id ?? href}>
              <Link
                href={href}
                className={`group flex h-full overflow-hidden rounded-[1.5rem] border border-slate-200/80 bg-white transition duration-300 hover:-translate-y-1 hover:border-primary/20 ${
                  isEmbedded
                    ? "shadow-[0_10px_28px_rgba(15,23,42,0.05)] hover:shadow-[0_18px_42px_rgba(74,108,247,0.12)]"
                    : "shadow-[0_14px_40px_rgba(15,23,42,0.06)] hover:shadow-[0_20px_55px_rgba(74,108,247,0.14)]"
                } ${
