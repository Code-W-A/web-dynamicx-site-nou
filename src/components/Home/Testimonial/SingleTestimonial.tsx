...
export default function SingleTestimonial({
  ...
  const Wrapper = ({ children }: { children: React.ReactNode }) =>
    testimonial?.href ? (
      <Link href={testimonial.href} className="hover:opacity-90">{children}</Link>
    ) : (
      <>{children}</>
    );
  ...
  return (
    <div className="w-full px-4 lg:w-1/2">
      <div className="relative mb-10 overflow-hidden rounded-xl bg-white p-10 px-7 shadow-testimonial sm:px-10">
        <div className="mb-5">
          <Wrapper>
            <h3 className="text-base font-semibold text-black">
              {testimonial?.companyName || testimonial?.name}
            </h3>
          </Wrapper>
        ...
        {testimonial?.href && (
          <div className="mt-6">
            <Link href={testimonial.href} className="text-primary inline-flex items-center gap-2 text-sm font-semibold hover:underline">
