...
export default function ServiceTabButtons({ serviceData }: any) {
  ...
  return (
    <div>
      <h3 className="mb-[22px] text-[34px] font-bold text-black">Servicii</h3>
      <div className="border-stroke rounded-xs border">
        {serviceData.map((service: Service) => (
          <Link
            href={`/servicii/${service?.slug}`}
            key={service?.id}
            className={`border-stroke flex w-full items-center gap-3 border-b px-5 py-4 text-lg font-medium duration-200 last-of-type:border-0 ${pathname === `/servicii/${service?.slug}` ? "text-primary" : "hover:text-primary text-black"}`}
          >
            <svg
              width="22"
              height="22"
