...
export async function generateMetadata(props: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      type: "article",
      images: [
        { url: ogImageUrl, alt: study.imageAlt, width: 1200, height: 630 },
      ],
    },
    twitter: {
    ...
    <>
      ...
      <JsonLd
        data={{
          ...
          url: `${siteURL}${mobilePortfolioHubPath}/${study.slug}`,
      ...
      <PageTitle
        pageTitle={study.h1}
        pageDescription={study.shortDescription}
      ...
      <article className="bg-white pb-20">
        <div className="container px-5 pt-6">
          <div className="mx-auto mb-8 max-w-4xl">
            <CaseStudyContact
              label={study.ctaLabel || "Vreau o aplicație similară"}
            />
            {study.status && (
              <p className="mt-4 text-sm text-slate-600">{study.status}</p>
            )}
          </div>
          ...
          {study.playStoreUrl ? (
            <div className="mx-auto mt-8 max-w-3xl">
              <a
                ...
                className="hover:border-primary/30 hover:text-primary inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-5 py-2.5 text-sm font-semibold text-slate-800 shadow-sm transition"
              >
                Vezi în Google Play
          ...
          {study.gallery && study.gallery.length > 0 ? (
            ...
            >
              ...
              <div className="mt-8 flex gap-6 overflow-x-auto overflow-y-visible pt-1 pb-4 [-ms-overflow-style:none] [scrollbar-width:none] sm:grid sm:grid-cols-2 sm:overflow-visible lg:grid-cols-3 [&::-webkit-scrollbar]:hidden">
          ...
          >
            ...
            <section>
              ...
              <ul className="mt-4 space-y-3">
                {study.impact.map((item) => (
                  ...
                  >
                    <span className="text-primary mt-0.5 shrink-0 font-semibold">
                      —
                    </span>
                    <span>{item}</span>
                  </li>
            ...
            <section className="rounded-[2rem] border border-slate-200/80 bg-[linear-gradient(180deg,_#f8fbff_0%,_#ffffff_100%)] p-8 sm:p-10">
              ...
              <p className="mt-4 text-base leading-relaxed text-slate-600">
                ...
                {study.nextStepOutro ??
                  ", apoi să ne scrii din formularul de contact."}
              </p>
              <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                ...
                <Link
                  ...
                  className="hover:text-primary inline-flex items-center justify-center gap-2 rounded-full border border-slate-200 px-6 py-3 text-sm font-semibold text-slate-800 transition"
                >
                  {study.primaryServiceCtaLabel ??
                    "Vezi cum lucrăm la aplicații mobile pentru business"}
                  <ArrowRight size={16} aria-hidden />
                </Link>
                <Link
                  href="/contact"
                  className="hover:border-primary/30 hover:text-primary inline-flex items-center justify-center rounded-full border border-slate-200 bg-white px-6 py-3 text-sm font-semibold text-slate-900 transition"
                >
                  Contact
                </Link>
                <Link
                  href={mobilePortfolioHubPath}
                  className="hover:text-primary inline-flex items-center justify-center text-sm font-semibold text-slate-700 underline-offset-4 transition hover:underline"
                >
