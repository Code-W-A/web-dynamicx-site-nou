...
export async function generateMetadata(props: PageProps): Promise<Metadata> {
  ...
  const title =
    lang === "en"
      ? "DO IT NOW — Privacy Policy & Terms"
      : "DO IT NOW — Politică de confidențialitate & Termeni";
  const description =
    lang === "en"
      ? "Legal page for the DO IT NOW mobile app: Privacy Policy and Terms & Conditions."
      : "Pagină legală pentru aplicația mobilă DO IT NOW: Politică de confidențialitate și Termeni și condiții.";

  return {
    title,
    description,
    alternates: {
      canonical: `${siteURL}/apps/do-it-now/legal`,
    },
    robots: {
      index: false,
      follow: false,
...
export default async function DoItNowLegalPage(props: PageProps) {
  const params = await props.searchParams;
  const lang = resolveLanguage(params?.lang);
  const content = legalContentByLang[lang];
  const basePath = "/apps/do-it-now/legal";
