...
export async function generateMetadata(props: PageProps): Promise<Metadata> {
  ...
  const title =
    lang === "en"
      ? "My Butterfly — Privacy Policy & Terms"
      : "My Butterfly — Politică de confidențialitate & Termeni";
  const description =
    lang === "en"
      ? "Legal page for the My Butterfly mobile app: Privacy Policy and Terms & Conditions."
      : "Pagină legală pentru aplicația mobilă My Butterfly: Politică de confidențialitate și Termeni și condiții.";

  return {
    title,
    description,
    alternates: {
      canonical: `${siteURL}/apps/my-butterfly/legal`,
    },
    robots: {
      index: false,
      follow: false,
...
export default async function MyButterflyLegalPage(props: PageProps) {
  const params = await props.searchParams;
  const lang = resolveLanguage(params?.lang);
  const content = legalContentByLang[lang];
  const basePath = "/apps/my-butterfly/legal";
    ...
    <section className="bg-white pb-20 pt-[140px]">
      <div className="container">
        <div className="mx-auto max-w-[860px]">
          ...
          <nav
            className="mb-10 rounded-xs border border-stroke bg-[#fafbff] p-5"
          ...
          >
            ...
            <ul className="space-y-2">
              {content.tocItems.map((item) => (
                <li key={item.id}>
                  <a
                    href={`${basePath}?lang=${lang}#${item.id}`}
          ...
          <article
            id="privacy-policy"
            className="scroll-mt-32 border-b border-stroke pb-10"
          >
